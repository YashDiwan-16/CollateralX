import { AlgorandClient, Config, algo, microAlgo } from "@algorandfoundation/algokit-utils"
import {
...
const SOURCE = new TextEncoder().encode("manual:testnet")
const RESERVE_SUPPLY = 10_000_000_000_000n

type SigningAccount = Awaited<ReturnType<AlgorandClient["account"]["fromEnvironment"]>>
...
async function loadRoleAccount(
  algorand: AlgorandClient,
  role: "KEEPER" | "ORACLE_UPDATER",
  fallback: SigningAccount
) {
  try {
    return await algorand.account.fromEnvironment(role)
  } catch {
    return fallback
  }
}
...
async function freshOracleClock(algorand: AlgorandClient) {
  ...
  if (timestamp === undefined || !Number.isFinite(timestamp)) {
    throw new Error("latest testnet block timestamp unavailable")
  }
...
async function main() {
  ...
  const keeper = await loadRoleAccount(algorand, "KEEPER", deployer)
  const oracleUpdater = await loadRoleAccount(algorand, "ORACLE_UPDATER", deployer)
  ...
  // App accounts need explicit balance headroom for global state, box storage,
  // and inner transactions. Funding here avoids first-user failures on testnet.
